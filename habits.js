document.addEventListener('DOMContentLoaded',()=>{
  ...
  function render(){
    ...
    habits.forEach((h, idx)=>{
      ...
      const head=document.createElement('div'); head.innerHTML = `<strong>${h.name}</strong> <button class="icon">✖</button>`;
      ...
      for(let i=0;i<7;i++){
        ...
        if(h.log && h.log[i]) cell.classList.add('active');
  ...
  addHabit?.addEventListener('click',()=>{
    ...
    habits.push({name, log:[]}); habitName.value=''; save(); render();
  });
  render();
});
